...
const generateCertificateCanvas = async (profile) => {
  ...
  const scoreValues = `
    <div style="${scoreValueStyle} left: ${mmToPx(150)}px; top: ${adjustY(133)}px;">: ${bestScores.listening}</div>
    <div style="${scoreValueStyle} left: ${mmToPx(150)}px; top: ${adjustY(139)}px;">: ${bestScores.structure}</div>
    <div style="${scoreValueStyle} left: ${mmToPx(150)}px; top: ${adjustY(144)}px;">: ${bestScores.reading}</div>
    <div style="${scoreTotalStyle} left: ${mmToPx(150)}px; top: ${adjustY(149.5)}px;">: ${bestScores.total}</div>
    <div style="${scoreRangeStyle} left: ${mmToPx(133)}px; top: ${adjustY(155.2)}px;">: 271-677</div>
  `;

  content.innerHTML = `
    <div style="${smallTextStyle} left: ${mmToPx(32)}px; top: ${adjustY(86)}px;">
      ${formatDate(profile.expired)}
    </div>
    <div style="${regisTextStyle} left: ${mmToPx(232)}px; top: ${adjustY(83)}px;">
      ${profile.nomor_registrasi}/${formatDateDDMMYYYY(profile.tanggal_selesai_ujian)}
    </div>
    <div style="${npsnTextStyle} left: ${mmToPx(232)}px; top: ${adjustY(88)}px;">
      <b>NPSN:</b> K9999499
    </div>
    <div style="${smallTextStyle} left: ${mmToPx(33)}px; top: ${adjustY(86)}px;">
      ${formatDate(formatDatePlusYears(Date.now(), 2))}
    </div>
    <div style="${nameTextStyle} left: ${mmToPx(148.5)}px; top: ${adjustY(100.2)}px; transform: translateX(-50%);">
      ${profile.nama.toUpperCase()}
    </div>
    <div style="${tempatLahirStyle} left: ${mmToPx(132.5)}px; top: ${adjustY(107.2)}px;">
      ${toTitleCase(profile.tempat_lahir)}, ${formatDate(profile.tanggal_lahir)}
    </div>
    <div style="${tempatLahirStyle} left: ${mmToPx(132.5)}px; top: ${adjustY(113.2)}px;">
      ${formatDate(profile.tanggal_selesai_ujian)}
    </div>
    ${scoreValues}
    <div style="${tanggalCetakStyle} left: ${mmToPx(146.5)}px; top: ${adjustY(160.1)}px;">
